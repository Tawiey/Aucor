        ...
        <div className="pt-32 pb-24 px-6 md:px-16 min-h-screen">
            <div className="max-w-7xl mx-auto">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-8 mb-16 border-b border-white/10 pb-8">
                    <div>
                        <h1 className="font-sans font-bold text-4xl md:text-5xl lg:text-6xl tracking-tight mb-4 text-ivory">
                            Curated <span className="font-drama italic font-normal text-accent">Properties</span>
                        </h1>
                        <p className="text-ivory/60 font-light max-w-lg">
                    ...
                    <form onSubmit={handleSearch} className="w-full md:w-auto min-w-[300px] relative">
                        ...
                        <input
                            ...
                            className="w-full bg-white/5 border border-white/10 rounded-full h-12 pl-12 pr-4 text-sm text-ivory font-sans outline-none focus:border-accent/50 focus:bg-white/10 transition-all placeholder:text-ivory/30"
                        />
                    </form>
                </div>
                ...
                <AnimatePresence mode="popLayout">
                    {filteredProperties.length === 0 ? (
                        <motion.div
                            ...
                            animate={{ opacity: 1 }}
                            exit={{ opacity: 0 }}
                            className="py-24 text-center border border-white/5 rounded-3xl bg-white/5"
                        >
                            <p className="font-mono text-ivory/40 uppercase tracking-widest text-sm mb-2">No results found</p>
                            <p className="text-ivory/60">Try adjusting your search criteria.</p>
                        </motion.div>
                    ) : (
                        <motion.div layout className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                            {filteredProperties.map((property, idx) => (
                                <motion.div
                                    ...
                                    key={property.id}
                                    className="group bg-obsidian border border-white/10 rounded-2xl overflow-hidden hover:border-white/20 transition-colors shadow-2xl flex flex-col"
                                >
                                    {/* Image Header */}
